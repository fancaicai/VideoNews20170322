package com.feicuiedu.fancaicai.videonews.ui.base;
...
public abstract class BaseResourceView<Model, ItemView extends BaseItemView<Model>> extends FrameLayout implements SwipeRefreshLayout.OnRefreshListener, MugenCallbacks {
    ...
    protected ModelAdapter mModelAdapter;
    ...
    private void initView() {
        //NewsApi初始化
        mNewsApi = BombClient.getInstance().getmNewsApi();
...
//        自定义下拉刷新的progressBar的颜色
        refreshLayout.setColorSchemeResources(R.color.blank);
//        配置上拉加载
    ...
    public void onRefresh() {
        ...
        call.enqueue(new Callback<QueryResult<Model>>() {
            ...
            public void onResponse(Call<QueryResult<Model>> call, Response<QueryResult<Model>> response) {
                ...
                //将数据添加到adapter
                mModelAdapter.clear();//刷新，清空旧数据
                mModelAdapter.addData((ArrayList<Model>) datas);
            }

            @Override
            public void onFailure(Call<QueryResult<Model>> call, Throwable t) {
                progressBar.setVisibility(View.INVISIBLE);//隐藏加载视图
    ...
    public void onLoadMore() {
        ...
        progressBar.setVisibility(View.VISIBLE);//显示加载视图
        call.enqueue(new Callback<QueryResult<Model>>() {
            @Override
            public void onResponse(Call<QueryResult<Model>> call, Response<QueryResult<Model>> response) {
                progressBar.setVisibility(View.INVISIBLE);//隐藏加载视图
